...
seq_data_cs1 <- read.table(sprintf("%s/PAS_source/findpolyAtail/findtail_PASs.bed", data_dir), sep = "\t") # 28421
summary(seq_data_cs1$V5)
seq_data_cs2 <- read.table(sprintf("%s/PAS_source/mws_cs_annotation/mws_cs_info.bed", data_dir), sep = "\t") # 21936
...
Annot_dir <- sprintf("%s/Annotation", data_dir)
...
#############################################
## PAS annotation ####
gene_annotation <- read.table(sprintf("%s/PAS_source/refSeq/refseq.annotation.txt", data_dir))
rownames(gene_annotation) <- gene_annotation$V5

system(sprintf("bedtools intersect -wo -s -a %s/Potential_PAS.bed -b %s/PAS_source/refSeq/last_exon_annotation.bed > %s/Potential_PAS_last_exon.bed", 
               Annot_dir, data_dir, Annot_dir))
...
system(sprintf("bedtools closest -s -D a -a %s -b %s > %s",
               sprintf("%s/PAS_source/findpolyAtail/high_abundance_IPS_sites.bed", data_dir),
...
high_abundance_around <- subset(high_abundance_IPS, abs(V23) < 100)
...
saveRDS(select_anno_res1, sprintf("%s/PAS_annotation.rds", Annot_dir))
...
# load GTF
gtf <- import(sprintf("%s/PAS_source/refSeq/GRCh38_rechr.gtf", data_dir))
exons <- gtf[gtf$type == "exon"]
exons_by_transcript <- split(exons, exons$transcript_id)


isoform_transcript <- read.table(sprintf("%s/PAS_source/refSeq/last_exon_annotation.bed", data_dir), sep = "\t")
...
PAS_structure_info_optimized <- function(tmp_index) {
    ...
    if (total_width >= 400) {
        ...
        remaining <- 400 - if (cut_index == 1) 0 else cum_widths[cut_index - 1]
        
        if (strand_dir == "+") {
            start(transcript_exons[cut_index]) <- end(transcript_exons[cut_index]) - remaining + 1
        } else {
            end(transcript_exons[cut_index]) <- start(transcript_exons[cut_index]) + remaining - 1
        }
    }
...
select_anno_tmp2 <- apply(select_anno_tmp2, 1, function(x){
    ...
    return(c(chr, start_v, end_v, x["site_name"], x["LE_name"], x["site_strand"], x["LE_start"], x["LE_end"]))
}) %>% t() %>% as.data.frame()
colnames(select_anno_tmp2) <- colnames(select_anno_tmp1)

## 1-based position
select_anno_tmp2$diff <- as.numeric(select_anno_tmp2$site_end)-as.numeric(select_anno_tmp2$site_start)+1
table(select_anno_tmp2$diff > 200)
select_anno_tmp2 <- select_anno_tmp2[select_anno_tmp2$diff > 200, ]
select_anno_tmp2$diff <- NULL

select_anno_tmp <- rbind(select_anno_tmp1, select_anno_tmp2)

len_num <- as.numeric(select_anno_tmp$site_end) - as.numeric(select_anno_tmp$site_start) + 1
range(len_num)
table(len_num < 200)

## GTF of PASs
select_anno_gr <- GRanges(seqnames = select_anno_tmp$site_chr,
                          ranges = IRanges(start = as.numeric(select_anno_tmp$site_start),
                                           end = as.numeric(select_anno_tmp$site_end),
                                           names = select_anno_tmp$site_name),
                          strand = select_anno_tmp$site_strand)
# select_anno_gr$transcript_id <- sapply(strsplit(select_anno_tmp$site_name, split = ":"), function(x){paste0(x[1:2], collapse = ":")})
# select_anno_gr$gene_id <- select_anno_gr$transcript_id
