load_bam_cell_anno3 <- function(bam_anno_file_="annots_1204.csv",
                                bam_dir_ = "cellranger/",
                                check_bam=FALSE){
  # pooled bam-cell-ind- details:
  bam_cell_anno <- fread(bam_anno_file_);
  names(bam_cell_anno)[which(names(bam_cell_anno)=="POOL")] <- "bam_id"
  uniq_ID <- unique(bam_cell_anno[,.(cluster, ind, bam_id)])
  uniq_ID <- cbind(data.table(cluster_ind_bam_id = paste0("splited_",1:nrow(uniq_ID))), uniq_ID)
  bam_cell_anno <- merge(bam_cell_anno, uniq_ID, by =c("cluster","ind", "bam_id"))
  # check whether cells from the same cell type belong to one individuals are split into different bams:
  # print(unique(bam_cell_anno[,.(cluster, ind, bam_id)])[,.(num_bam = nrow(.SD)), by =c("cluster", "ind")][order(num_bam)])
  
  # pooled bam file path:
  if(check_bam){
    bam_dir <- c( list.dirs(bam_dir_, recursive = FALSE))
    bam_dir <- data.table(bam_id = basename(bam_dir), bam_file_path = normalizePath(paste0(bam_dir, "/outs/possorted_genome_bam.bam")))
    bam_dir <- bam_dir[bam_id %in% unique(bam_cell_anno$bam_id)]
...
remove_dups_pools2 <- function(bam_anno_=bam_anno, 
                               parThreads_=30, out_path ="",
                               umitools_exe_="umi_tools", samtools_exe_="/usr/bin/samtools"){
  start_time_i <- Sys.time()
  message("==> total: ",nrow(bam_anno_))
  bam_anno_[,c("bam_dedup_path"):=.( str_replace_all(paste0(outDir, "/bam_tmp/",str_remove(bam_file_path, cellrangerDir)),stringr::fixed("//"), stringr::fixed("/")) )]
  bam_anno_[,c("bam_dedup_path"):=.( str_replace_all(bam_dedup_path, "possorted_genome_bam.bam", "possorted_genome_dedup.bam") )]
  # bam_anno_$bam_dedup_path <- paste0(dirname(bam_anno_$bam_file_path),"/possorted_genome_dedup.bam")
  bam_anno_$dedup_exist <- unlist(lapply(1:nrow(bam_anno_),function(i){
    bam_path_i <- bam_anno_[i,]$bam_file_path
  ...
  bam_anno_ <- bam_anno_[dedup_exist==0,]
  message("==> left: ",nrow(bam_anno_))
  
  a <- mclapply(1:nrow(bam_anno_), function(i){
    message("==> ",i, " | ", bam_anno_[i,]$bam_id, " | ",bam_anno_[i,]$cluster, " | ",bam_anno_[i,]$ind, " | ",date())
    bam_path_i <- bam_anno_[i,]$bam_file_path
    bam_dedup_path_i <- bam_anno_[i,]$bam_dedup_path
    ...
    if( !file.exists(bam_dedup_path_i) | ifelse(is.na(file.size(bam_dedup_path_i)), 0, file.size(bam_dedup_path_i)) == 0  ){
      ...
      system(paste0(umitools_exe_, " dedup -I ", bam_path_i, " -S ", bam_dedup_path_i, " --per-cell --method=unique --extract-umi-method=tag --umi-tag=UB --cell-tag=CB "))
    }
    # create index:
    if(file.exists(bam_dedup_path_i) & !file.exists(paste0(bam_dedup_path_i,".bai"))){
      system(paste0(samtools_exe_, " index -@ ",4," ", bam_dedup_path_i, " "))
    }
    return(running_time)
...
split_dedup_of_pool_bam <- function(bam_anno_=bam_anno, out_dir_ = "", 
  ...
  bam_anno_[,c("bam_dedup_path"):=.( str_replace_all(paste0(outDir, "/bam_tmp/",str_remove(bam_file_path, cellrangerDir)),stringr::fixed("//"), stringr::fixed("/")) )]
  bam_anno_[,c("bam_dedup_path"):=.( str_replace_all(bam_dedup_path, "possorted_genome_bam.bam", "possorted_genome_dedup.bam") )]
  bam_anno_ <- bam_anno_[file.exists(bam_dedup_path)]
  if(nrow(bam_anno_)==0){return()}
  bam_anno_$split_exist <- unlist(lapply(1:nrow(bam_anno_),function(i){
    bam_cell_anno_i <- bam_cell_anno[bam_id == bam_anno_[i,]$bam_id,]
    out_dir_i <- paste0(out_dir_, bam_anno_[i,]$bam_id)
    if(length(setdiff(paste0(unique(bam_cell_anno_i$cluster_ind_bam_id), ".bam"), list.files(out_dir_i))) == 0){
  ...
  bam_anno_ <- bam_anno_[split_exist==0,]
  message("==> left: ",nrow(bam_anno_))
  if(nrow(bam_anno_)==0){return(0)}
  # 6 batches, each 12 threads:
  start_time_i <- Sys.time()
  run_time <- mclapply( 1:nrow(bam_anno_), function(i){
    message("==> | ",i, " | ", bam_anno_[i,]$bam_id, " | ",date())
    out_dir_i <- paste0(out_dir_, bam_anno_[i,]$bam_id)
    cell_id_path_i <- paste0(out_dir_, bam_anno_[i,]$bam_id,"/bam_cell_anno.txt")
    if(!dir.exists(out_dir_i)){ dir.create(out_dir_i, recursive = TRUE) }
    bam_cell_anno_i <- bam_cell_anno[bam_id == bam_anno_[i,]$bam_id,]
    # write out
    fwrite(bam_cell_anno_i[,.(cell_id, cluster_ind_bam_id)], cell_id_path_i, sep="\t", col.names = FALSE)
    ...
    system(paste0( sinto_exe_," filterbarcodes -p ",subThreads_," -b ", bam_dedup_path_i, " -c ", cell_id_path_i, " --outdir ", out_dir_i))
    message("==> ",i, " | ", bam_anno_[i,]$bam_id," | done! ",date())
    return()
  }, mc.cores = parThreads_)
  end_time_i <- Sys.time()
  running_time <- as.numeric(difftime(end_time_i, start_time_i,units = "mins"))
  return(unlist(run_time))
}
...
combine_bam_ind <- function(bam_cell_anno_=bam_cell_anno, 
                            ...
                            samtools_exe_="samtools"){
  if(!dir.exists(out_dir_)){ dir.create(out_dir_, recursive = TRUE)}
  if(!dir.exists(out_dir_)){stop("==> ",out_dir_, " | can not be created")}
  inds <- unique(bam_cell_anno[,.(ind)])
  
  start_time_i <- Sys.time()
  a <- mclapply(1:nrow(inds), function(i){
    message("==> i:",i," | ",date())
    ind_i <- inds[i,]$ind
    bam_cell_anno_i <- unique(bam_cell_anno_[ind==ind_i,.(cluster, ind, bam_id, cluster_ind_bam_id)])
    bam_cell_anno_i$bam_splited_path <- paste0(bam_splited_dir_, bam_cell_anno_i$bam_id ,"/",bam_cell_anno_i$cluster_ind_bam_id,".bam" )
    ...
    bam_cell_anno_i$cluster_info <- paste0(bam_splited_dir_,"/", bam_cell_anno_i$bam_id, "/bam_cell_anno.txt" )
    cluster_info_i <- rbindlist(lapply(1:nrow(bam_cell_anno_i), function(k){ a<-fread(bam_cell_anno_i[k]$cluster_info, header = FALSE);names(a)<-c("cell_id", "cluster_ind_bam_id");a$bam_id <- bam_cell_anno_i[k]$bam_id;return(a[cluster_ind_bam_id==bam_cell_anno_i[k]$cluster_ind_bam_id]) }))
    cluster_info_out_paht_i <- paste0(out_dir_,"/", inds[i,]$ind,"_cell_info.txt")
    ...
    if( !file.exists(bam_ind_out_path_i) | ifelse(is.na(file.size(bam_ind_out_path_i)), 0, file.size(bam_ind_out_path_i)) == 0 ){
      ...
      if(nrow(bam_cell_anno_i)>1){
        system(paste0(samtools_exe_, " merge -f -@ ",subThreads_," -O bam ", bam_ind_out_path_i, " ",
    ...
    if( file.exists(bam_ind_out_path_i) & !file.exists( paste0(bam_ind_out_path_i, ".bai") )){
      ...
      system(paste0(samtools_exe_," index -@ ",subThreads_," ", bam_ind_out_path_i, " &"))
    }
  }, mc.cores = parThreads_)
  end_time_i <- Sys.time()
  running_time <- as.numeric(difftime(end_time_i, start_time_i,units = "mins"))
...
combine_bam_celltype_ind <- function(bam_cell_anno_=bam_cell_anno, 
                                     bam_splited_dir_ = "", 
                                     out_dir_ = "", 
                                     parThreads_=15, subThreads_=4, 
                                     samtools_exe_="samtools"){
  start_time_i <- Sys.time()
  if(!dir.exists(out_dir_)){dir.create(out_dir_, recursive = TRUE)}
  if(!dir.exists(out_dir_)){ stop("==> ",out_dir_, " | can not be created.." ) }
  cell_types <- unique(bam_cell_anno_[,.(cluster)])
  for(i in 1:nrow(cell_types)){
    message("==> i:",i," | ",date())
    bam_cell_anno_i <- unique(bam_cell_anno_[cluster==cell_types[i,]$cluster,.(cluster, ind, bam_id, cluster_ind_bam_id)])
    inds_i <- unique(bam_cell_anno_i[,.(ind)])
    # parallel:
    a <- mclapply(1:nrow(inds_i), function(j){
      message(j)
      bam_ind_out_path_j <-  paste0(out_dir_,"/", cell_types[i,]$cluster,"/",inds_i[j,]$ind,".bam")
      cluster_info_out_paht_j <- paste0(out_dir_,"/", cell_types[i,]$cluster,"/",inds_i[j,]$ind,"_cell_info.txt")
      bam_ind_dedup_out_path_j <-  paste0(out_dir_,"/", cell_types[i,]$cluster,"/",inds_i[j,]$ind,"_dedup.bam")
      if(!dir.exists(dirname(bam_ind_out_path_j))){ dir.create(dirname(bam_ind_out_path_j), recursive = TRUE) }
      bam_cell_anno_j <- unique(bam_cell_anno_i[ind==inds_i[j,]$ind,.(ind, cluster, bam_id, cluster_ind_bam_id)])
      bam_cell_anno_j$bam_splited_path <- paste0(bam_splited_dir_,"/", bam_cell_anno_j$bam_id, "/", bam_cell_anno_j$cluster_ind_bam_id, ".bam")
      bam_cell_anno_j$cluster_info <- paste0(bam_splited_dir_,"/", bam_cell_anno_j$bam_id, "/bam_cell_anno.txt" )
      ...
      cluster_info_j <- rbindlist(lapply(1:nrow(bam_cell_anno_j), function(k){ a<-fread(bam_cell_anno_j[k]$cluster_info, header = FALSE);names(a)<-c("cell_id", "cluster_ind_bam_id");a$bam_id <- bam_cell_anno_j[k]$bam_id;return(a[cluster_ind_bam_id==bam_cell_anno_j[k]$cluster_ind_bam_id]) }))
      ...
      if(!file.exists(bam_ind_out_path_j) | ifelse(is.na(file.size(bam_ind_out_path_j)), 0, file.size(bam_ind_out_path_j)) == 0 ){
        message(" ==>File does not exist: ",cell_types[i,]$cluster, " | "," | ",inds_i[j,]$ind, " | start merge: ")
        if(nrow(bam_cell_anno_j)>1){
          system(paste0(samtools_exe_, " merge -f -@ ",subThreads_," -O bam ", bam_ind_out_path_j, " ",
                        paste0(bam_cell_anno_j$bam_splited_path, collapse = " ")))
        }else{
          system(paste0("cp ",bam_cell_anno_j[1,]$bam_splited_path, " ", bam_ind_out_path_j))
        }
      }
      # make index for merged bam file:
      if( file.exists(bam_ind_out_path_j) & !file.exists( paste0(bam_ind_out_path_j, ".bai") )){
        message( " ==>", " - ",cell_types[i,]$cluster,"  - ",inds_i[j,]$ind, " | create index:")
        system(paste0(samtools_exe_," index -@ ",subThreads_," ", bam_ind_out_path_j, " &"))
      }
      return(j)
    }, mc.cores = parThreads_)
  }
...
# -s 1 read1 as RNA,read2 not as RNA; single-cell的没有read2呀，所以 s=1和s=12的结果是一样的！
# -E Exclude positions with multiple changes
# -v Minimum number of reads supporting the variation [3], 貌似这个值只影响Frequence,不影响输出结果， 但会根据-n进行AllSubs=="-"，如果-n 0的话就不会过滤，同时 -v 1的话
...
detect_editing_by_reditools_13m_noSNP_for_ind <- function(bam_cell_anno_= bam_cell_anno, 
  ...
  # 
  bam_ind <- unique(bam_cell_anno_[,.(ind)])
  bam_ind$bam_path <- paste0(bam_ind_dir_,"/",bam_ind$ind,".bam")
  bam_ind$RE_out_path <- paste0(RE_out_dir_, bam_ind$ind)
  ...
  Sys.sleep(1)
  # 
  a <- mclapply(1:nrow(bam_ind), function(i){
    ind_i <- bam_ind[i,]$ind
...
summary_ed_cov_cellNum_for_cell_type_ind_pool <- function(bam_cell_anno_=bam_cell_anno,
                                                          RE_out_reditools_ ="reditools_13m_ind/",
                                                          bam_split_by_celltype_ind_path = "bam_split_by_celltype_ind/",
                                                          ref_fa_ = "genome.fa",
                                                          out_dir= "reditools_13m_ind_celltype/",
                                                          min_mapping_quality=0,
                                                          bed_splited_lines=1000,
                                                          parThreads_=40,
                                                          samtools_exe_="/usr/bin/samtools"
){
  start_time_i <- Sys.time()
  bam_ind <- unique(bam_cell_anno_[,.(ind)])
  bam_ind$RE_out_path <- paste0(RE_out_reditools_, bam_ind$ind,"_refine.txt")
  bam_ind <- bam_ind[file.exists(bam_ind$RE_out_path),]
  inds <- unique(bam_ind[,.(ind)])
  a <- lapply(1:nrow(inds), function(i){
    ind_i <- inds[i,]$ind
    bed_path_i <- str_replace(bam_ind[ind==ind_i]$RE_out_path, "_refine.txt", ".bed")
    # if(file.exists(bed_path_i)){message("==> ",i," | output exists! next");return()}
    
    # editing for ind:
    re_i <- fread(bam_ind[ind==ind_i]$RE_out_path, header=TRUE, sep="\t")
    ...
    # bam file of cell type for ind:
    # bam_cell_anno_i <- bam_cell_anno_[ind==ind_i,]
    cell_types_i <- unique(bam_cell_anno_[ind==ind_i,][,.(cluster)])
    cell_types_i$bam_path <- paste0(bam_split_by_celltype_ind_path, cell_types_i$cluster,"/", ind_i,".bam")
    ...
    a <- lapply(1:nrow(cell_types_i), function(j){
      message("==> ",i," | ",j)
      cell_type_j <- cell_types_i[j]$cluster
      bam_path_j <- cell_types_i[j]$bam_path
      cell_info_out_dir_j <- paste0(out_dir, cell_type_j,"/",ind_i,"/")
      ...
      a <- refine_summary_pileup_poolID_Nathan(bam_cell_anno_ = bam_cell_anno,
                                               ...
                                               parThreads_= parThreads_)
      # extract mmp file
      a <- cell_reads_summary(cell_reads_path = cell_reads_path_j,
...
extract_cell_infomation_poolID <- function(pos_bed_ ="ed_refined.bed",
  ...
  if(pos_bed_==""){
    # 
    # bam_file_ <- "/media/pacific/zhangt/single_cell_datasets/Randolph2021Science/scRNAseq/bam_split_by_ind/HMN171215.bam"
    # cell_info_out_dir_ <- "/media/bora_A/dingruofan/2023-10-16_RNA_editing_project/outputs/Randolph2021Science/reditools_13m_ind_denovo_pileup/HMN171215_all_region/"
    # samtools_exe_ <- "/usr/bin/samtools"
    # ref_fa_ = "/media/bora_A/zhangt/src/hg38_cellranger_index/fasta/genome.fa"
    ...
    mclapply(paste0("chr", c(1:22, "X", "Y") ), function(c){
      print(c)
      system(paste0( samtools_exe_, " view -b ",bam_file_, " ",c," >",bam_splited_dir,c,".bam" ))
      return()
    }, mc.cores = parThreads_)
    ...
    a <- mclapply(1:length(splited_bams), function(i){
      ...
      suppressMessages(system(paste0(samtools_exe_, " mpileup --output-extra CB -B -d 100000 -q ",min_mapping_quality," ", bam_i, " -f ", ref_fa_, " | awk '{if($4>=10 && $5 ~\"[ATGCatgc]\" )print}' >", pileup_out_i)))
      return(NULL)
    ...
    # split bed file:
    system(paste0("cd ", bed_splited_dir,"; split -l ",bed_splited_lines," ", pos_bed_))
    ...
    a <- mclapply(1:length(splited_beds), function(i){
      ...
      suppressMessages(system(paste0(samtools_exe_, " mpileup --output-extra CB,RG -A -B -d 100000 -q ",min_mapping_quality," ", bam_file_, " -f ", ref_fa_, " -l ", bed_i," -o ", pileup_out_i)))
      return(NULL)
    }, mc.cores = parThreads_)
    # 
    message(" ==> Combine splited pileup files:")
...
refine_summary_pileup_poolID_Nathan <- function(bam_cell_anno_ = bam_cell_anno,
  ...
  if(type=="ed"){
    ...
    bam_cell_anno_x <- bam_cell_anno_[cluster==cluster_x & ind == ind_x,.(cluster, ind, bam_id, cell_id)]
    setindex(bam_cell_anno_x, cell_id)
    
    pileup <- fread(pileup_path_, sep="\t", header=FALSE)
    if(nrow(pileup)==0){return()}
    names(pileup) <- c("chrom", "position", "ref", "readCount", "bases","baseQuality", "cell_id", "ReadGroup")
    ...
    pileup$cell_id <- unlist(mclapply(1:nrow(pileup), function(i){
      cell_id_splited <- str_split_1(pileup[i,]$cell_id, ",")
    ...
    pileup <- rbindlist(mclapply(1:nrow(pileup), function(i){
      ...
      if(all(quality_score_i>=base_quality_score_cutoff_)){
        ...
        return(pileup[i,][,.(chrom, position, ref, readCount=length(bases_good_i), bases=paste0(str_split_1(bases, "")[bases_good_i],collapse = ""), baseQuality = paste0(str_split_1(pileup[i]$baseQuality,"")[bases_good_i], collapse = ""), cell_id = paste0(str_split_1(cell_id, ",")[bases_good_i], collapse = ""), removed_reads=length(quality_score_i)-length(which(quality_score_i>=base_quality_score_cutoff_)) )])
      }
    }, mc.cores = parThreads_))
    # message(" ==> removed ", nrow(pileup[bases==""]), "/", nrow(pileup))
    pileup <- pileup[bases!=""]
    ...
    # 每个位点如果有两条链的突变的话就保留两行的结果,统计 :
    # foward reads by cell
    
    # procell ReadGroup
    # combine cell_id with ReadGroup:
    pileup$rg_cell_id <- unlist(mclapply(1:nrow(pileup), function(i){
      cell_id_i <- unlist(str_split(pileup[i,]$cell_id,",")[[1]])
      cell_id_splited_dt <- bam_cell_anno_x[cell_id_i, on="cell_id"]
      cell_id_splited_dt[is.na(bam_id), "bam_id"] <- na.omit(cell_id_splited_dt[1,])$bam_id
      cell_id_splited_dt <- cell_id_splited_dt[1:length(cell_id_i),]
      return(paste0(paste0(cell_id_splited_dt$bam_id, "#", cell_id_splited_dt$cell_id), collapse = ","))
    }, mc.cores = parThreads_))
    #
    cell_reads <- rbindlist(mclapply(1:nrow(pileup), function(i){
      pileup_i <- pileup[i,]
      cell_reads_i <- as.data.table(table(pileup_i[,.(bases=str_split_1(pileup_i$bases, ""), cell_id = str_split_1(pileup_i$rg_cell_id, ","))]))[N>0]
      names(cell_reads_i) <- c("base_pileup", "cell_id", "num_reads")
...
cell_reads_summary <- function(cell_reads_path = "cell_reads.txt",
  ...
  if(type=="deNovo"){
    ...
    misMatch_pop_each_cell <- rbindlist(mclapply(1:parThreads, function(i){
      cell_reads[phenotypes[batch==i,]$phenotype_id, on="phenotype_id"][,.(misMatch_reads=sum(.SD[base_pileup!=ref]$num_reads), cov_reads=sum(.SD$num_reads) ),by=c("cell_id", "phenotype_id")]
    }, mc.cores = parThreads))
    misMatch_pop_each_cell[,c("misMatch_prop"):=.(misMatch_reads/cov_reads )]
    misMatch_pop_each_cell <- merge(misMatch_pop_each_cell, phenotypes[,-c("batch")], by= "phenotype_id", sort=FALSE )[,.(phenotype_id, chrom, position,strand, cell_id, misMatch_reads,cov_reads, misMatch_prop)]
    ...
    #   misMatch_pop_each_cell_i <- cell_reads_i[,.(misMatch_reads=sum(.SD[base_pileup!=ref]$num_reads), cov_reads=sum(.SD$num_reads) ),by="cell_id"]
    #   misMatch_pop_each_cell_i[,c("misMatch_prop"):=.(misMatch_reads/cov_reads )]
    #   return(cbind( unique(cell_reads_i[,.(chrom, position, strand, phenotype_id)]), misMatch_pop_each_cell_i)[,.(phenotype_id, chrom, position,strand, cell_id, misMatch_reads,cov_reads, misMatch_prop)] )
    # }, mc.cores = parThreads))
  ...
  if(type=="ed"){
    ...
    misMatch_pop_each_cell <- rbindlist(mclapply(1:nrow(ed), function(i){
      ...
      if( ed_i$strand=="-" ){
        ...
        misMatch_pop_each_cell_i <- cell_reads_i[,.(misMatch_reads=sum(.SD[base_pileup!=","]$num_reads), cov_reads=sum(.SD$num_reads) ),by="cell_id"][order(misMatch_reads)]
    ...
    # 保留位点来自所有细胞的 cov reads num>=3
    gt_cell_reads <- merge(gt_cell_reads, gt_cell_reads[,.(cell_reads=sum(num_reads)),by=c("cell_id", "phenotype_id")][cell_reads >= cov_reads_cutoff,-c("cell_reads")], by=c("cell_id", "phenotype_id"), sort=FALSE)
    # 一个位点所有细胞中至少有一个错配，这样才能说明这是个突变
    gt_cell_reads <- merge(gt_cell_reads,  gt_cell_reads[base_pileup!="." & base_pileup!="," ][,.(cell_reads=sum(num_reads)),by= c("cell_id", "phenotype_id")][cell_reads >= mismatch_reads_cutoff], by=c("cell_id", "phenotype_id"), sort=FALSE )
    # 一个位点至少有1个细胞：
    gt_vars <- gt_cell_reads[,.(num_cells = length(unique(cell_id)) ), by="phenotype_id"][num_cells >= min_cov_cells_per_site]
    ...
    mutated_pop_each_cell <- rbindlist(mclapply(1:parThreads, function(i){
      gt_cell_reads[phenotypes[batch==i,]$phenotype_id, on="phenotype_id"][,.(misMatch_reads=sum(.SD[base_pileup!=ref]$num_reads), cov_reads=sum(.SD$num_reads) ),by=c("cell_id", "phenotype_id")]
    }, mc.cores = parThreads))
    mutated_pop_each_cell[,c("misMatch_prop"):=.(misMatch_reads/cov_reads )]
    mutated_pop_each_cell <- merge(mutated_pop_each_cell, phenotypes[,-c("batch")], by= "phenotype_id", sort=FALSE )[,.(phenotype_id, chrom, position, cell_id, misMatch_reads,cov_reads, misMatch_prop, gt)]
    ...
    #   # 统计至少有一个 mutated reads的细胞比例：
    #   misMatch_pop_each_cell_i <- gt_cell_reads_i[,.(misMatch_reads=sum(.SD[base_pileup !="." & base_pileup !=","]$num_reads), cov_reads=sum(.SD$num_reads) ),by="cell_id"][order(misMatch_reads)]
    #   misMatch_pop_each_cell_i$misMatch_prop <- misMatch_pop_each_cell_i$misMatch_reads / misMatch_pop_each_cell_i$cov_reads
...
# filter edsites for each cell type each ind (>=3 cells per site)
filter_ed_by_cell_and_cov_ind <- function(bam_cell_anno_=bam_cell_anno,
                                          RE_out_reditools_ ="/media/bora_A/dingruofan/2023-10-16_RNA_editing_project/outputs/Randolph2021Science/reditools_13m_ind/",
                                          mmp_out_dir= "/media/bora_A/dingruofan/2023-10-16_RNA_editing_project/outputs/Randolph2021Science/reditools_13m_ind_celltype/",
  ...
  bam_ind <- bam_ind[file.exists(bam_ind$RE_out_path),]
  inds <- unique(bam_ind[,.(ind)])
  clusters <- unique(bam_cell_anno_[,.(cluster)])
  lapply(1:nrow(clusters), function(i){
    cluster_i <- clusters[i]$cluster
    ...
    a <- mclapply(1:nrow(inds), function(j){
      ...
      ed_j_1 <- mmp_j[phenotype_id %in% mmp_j[phenotype_id %in% mmp_j[misMatch_prop>0]$phenotype_id,][,.(cell_num=length(cell_id)), by="phenotype_id"][cell_num>= min_num_covered_cells, ]$phenotype_id ]
      # calculate editing level:
      ed_j_1 <- ed_j_1[,.(cell_num=length(cell_id),cov_reads=sum(cov_reads), edited_reads = sum(misMatch_reads),ed_level = sum(misMatch_reads) / sum(cov_reads)), by="phenotype_id"]
      ...
      # # compare with last result derived from psuedobulk of cell type + ind:
      ...
      ed_j_0 <- mmp_j[phenotype_id %in% mmp_j[phenotype_id %in% mmp_j[,.( ed_level=sum(misMatch_reads) / sum(cov_reads) ), by="phenotype_id"][ed_level==0]$phenotype_id,][,.(cell_num=length(cell_id)), by="phenotype_id"][cell_num>= min_num_covered_cells_0, ]$phenotype_id ]
      # calculate editing level:
      ed_j_0 <- ed_j_0[,.(cell_num=length(cell_id),cov_reads=sum(cov_reads), edited_reads = sum(misMatch_reads),ed_level = sum(misMatch_reads) / sum(cov_reads)), by="phenotype_id"]
      ...
      fwrite(ed_filtered_cells[,.(phenotype_id, chrom, position=as.character(as.integer(position)), strand, ed_type, cell_num, cov_reads, edited_reads, ed_level )], 
...
merge_eds_ind_filtered <- function(bam_cell_anno_= bam_cell_anno, 
      ...
      bam_cluster <- unique(bam_cell_anno_[,.(ind, cluster)])
      bam_cluster$ed_path <- paste0(mmp_out_dir, bam_cluster$cluster,"/",bam_cluster$ind,"/ed_filtered_cells.txt")
      bam_cluster <- bam_cluster[file.exists(bam_cluster$ed_path)]
      #
      inds <- unique(bam_cluster[,.(ind)])
      clusters <- unique(bam_cell_anno_[,.(cluster)])
      #
      a<-lapply(1:nrow(clusters), function(i){
        ...
        bam_cluster_i <- bam_cluster[cluster== cluster_i]
        # 注意，这里每个cluster的人可能不一样！所以得单独计算！
        min_num_of_ind_i <- round(prop_of_ed_shared_inds *nrow(bam_cluster_i))
        eds_i <- rbindlist(mclapply(1:nrow(bam_cluster_i), function(j){
          ed_j <- fread(bam_cluster_i[j]$ed_path); ed_j$ind <- bam_cluster_i[j]$ind
          # if(only_AG){ed_j <- ed_j[ed_type=="AG"]}
          ed_j <- ed_j[chrom %in% paste0("chr",1:22)]
          return(ed_j)
        }, mc.cores = parThreads_merge_))
        if(nrow(eds_i)==0){ message(" ==> no data!"); return()}
        # filter ed by ind num:
        if(filter_ed_gt_0){
          eds_i <- eds_i[phenotype_id %in% eds_i[ed_level>0,][,.(num_ind= length(ind)), by="phenotype_id"][num_ind>=min_num_of_ind_i]$phenotype_id,]
        }else{
          eds_i <- eds_i[phenotype_id %in% eds_i[,.(num_ind= length(ind)), by="phenotype_id"][num_ind>=min_num_of_ind_i]$phenotype_id,]
        }
        # filter ed by ind num( ed level>0):
        ...
        a <- data.table( matrix(inds$ind, nrow=1) )
        names(a) <- unique( inds$ind )
        a <- a[0,]
        ...
        a <- mclapply(1:length(chroms_i), function(j){
          ...
          eds_dcast_j <- dcast(eds_j, chrom+position+end+phenotype_id~ind, value.var = "ed_level", fun.aggregate = function(x) if(length(x) == 0) NA_real_ else sum(x, na.rm = TRUE) )
