...
## read support >=2 in each sample, read support >= 20 in all studies and at least two studies.
...
total_read_support <- 20
total_source_support <- min(max(tmp_aggr_res$support), 2)

tmp_aggr_clean <- subset(tmp_aggr_res, count >= total_read_support & 
                             support >= total_source_support)
